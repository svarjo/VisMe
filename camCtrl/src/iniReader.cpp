 ...
 */
void getCameraIds(std::vector<std::string> &idStrings,
...
void getSaveSettings(saveSettings_t *pSet, const std::string& p_filename) {
	...
	if (rval == -2) {
		std::cout << "Output path [" << pSet->outPath
				<< "] already exist!\n\tAre you sure to use old path? y/N"
				<< std::endl;
		char key = getchar();
		if (key == 'y' || key == 'Y') {
...
/**
 * Populate adpative capture settings from ini-file
 */
void getAdaptiveSettings(adaptiveSettings_t *pSet, const std::string& p_filename)
{
	...
	minIni ini(p_filename);
	pSet->maxExpTime = ini.geti("adaptive", "maxExpTime", 500000);		//Max exposure time (too large -> noise)
	pSet->minExpTime = ini.geti("adaptive", "minExpTime", 15);			//Minumum exposure time (from HW GT1290 15µs)
	pSet->expTargetMeanValue = ini.geti("adaptive", "expTargetMeanValue", 6000);//OK exposure target (mean from image )
	pSet->expTimeFraction = ini.getf("adaptive", "expTimeFraction", 20); //#auto exposure solved is divided by this value for HDR excess light capture
	pSet->integrationTargetMean = ini.getf("adaptive", "integrationTargetMean", 163830); //#auto exposure solved is divided by this value for HDR excess light capture
 ...
 */
void getExperimentSettings(experimentSettings_t *pSet,
		const std::string& p_filename) {
	if (!FileIO::parameterFileExists(p_filename)) {
		exit(-1);
	}
	...
	std::cout << "Capturing images every: " << pSet->captureInterval
			<< " second" << std::endl;

	pSet->saveSumImage = ini.getbool("ImageStackExpTime", "saveSumImage",true);
	if (pSet->saveSumImage)
		std::cout << "Saving sum image of image stack" << std::endl;

	pSet->saveStackImages = ini.getbool("ImageStackExpTime", "saveStackImages",true);
	if (!pSet->saveStackImages)
		std::cout << "Exposure stack images are not saved!" << std::endl;
	...
	case IMAGE_STACK_EXPTIME: {
		...
		for (int imageId = 0; imageId < Nimages; imageId++) {
			if (p == NULL) {
				std::cout
						<< "Unexpcted end of ExposureTimes string in [ImageStackExpTime]"
						<< std::endl;
				exit(-1);
			}
			exptime = atof(p);
	...
	std::cout << "\tautogain = " << camBaseSettings.autogain << " | gain value ="
			<< camBaseSettings.gain << std::endl;
	std::cout << "\tautoexposure = " << camBaseSettings.autoexposure
			<< " | expTime=" << camBaseSettings.exposureTime << std::endl;
	std::cout << "\tautowhitebalance = " << camBaseSettings.autowhitebalance
			<< std::endl;
	std::cout << "\tautoiris = " << camBaseSettings.autoiris << " | value="
			<< camBaseSettings.iris << std::endl;
	std::cout << "\tgamma val = " << camBaseSettings.gamma << std::endl
			<< std::endl;
